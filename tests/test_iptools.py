...
async def test_subnet_split_counts_arithmetically_without_materializing():
    ...
    data = await _call(
        "subnet_split", {"prefix": "10.0.0.0/8", "new_prefix_length": 32}
    )
...
async def test_subnet_split_small():
    data = await _call(
        "subnet_split", {"prefix": "10.0.0.0/24", "new_prefix_length": 26}
    )
    assert data["total"] == 4
    assert data["subnets"] == [
        "10.0.0.0/26",
...
async def test_ip_contains_subnet():
    data = await _call(
        "ip_contains", {"network": "192.168.0.0/16", "address": "192.168.1.0/24"}
    )
    assert data["contains"] is True
...
async def test_ip_contains_mixed_version_prefix():
    data = await _call(
        "ip_contains", {"network": "10.0.0.0/8", "address": "2001:db8::/32"}
    )
    assert data["contains"] is False
...
async def test_prefix_overlap_contains():
    data = await _call(
        "prefix_overlap", {"prefix_a": "10.0.0.0/24", "prefix_b": "10.0.0.128/25"}
    )
...
async def test_prefix_overlap_disjoint():
    data = await _call(
        "prefix_overlap", {"prefix_a": "10.0.0.0/24", "prefix_b": "10.0.1.0/24"}
    )
...
async def test_prefix_overlap_mixed_version_disjoint():
    data = await _call(
        "prefix_overlap", {"prefix_a": "10.0.0.0/8", "prefix_b": "2001:db8::/32"}
    )
    assert data["overlaps"] is False
    assert data["relationship"] == "disjoint"
